(function() {
    ...
    function validateFields(fields) {
        for (const f of fields) {
            if (f.required) {
                ...
                if (!el || !el.value.trim()) {
                    alert(`${f.label} आवश्यक है। कृपया भरें।`);
                    el && el.focus();
                    return false;
                }
            }
    ...
    generateDocBtn.addEventListener('click', function() {
        if (!currentTemplateData) return alert('कोई टेम्पलेट लोड नहीं है।');
        if (!validateFields(currentTemplateData.fields || [])) return;
    ...
    newDocBtn.addEventListener('click', () => {
        showScreen('screen1');
        docPreview.innerHTML = `<div class="placeholder-text"><i class="fas fa-file-alt" style="font-size:40px; display:block; color:#ddd;"></i> नया दस्तावेज़ जनरेट करें</div>`;
    });
